...
class WorkflowExecutor(QThread):
    ...
    def run(self):
        try:
            ...
            success, _ = self._run_subprocess(script)
    ...
    def _generate_execution_script(self, sorted_nodes: list, data_graph: dict, flow_graph: dict) -> str:
        ...
        for node_key in sorted_nodes:
            ...
            code += f"    print(f'🔴 Error in {fqnn}:', tb, flush=True)\n"
...
def custom_input(prompt=''):
    print(f"__PYWORKS_INPUT_PROMPT__:{{prompt}}", flush=True)
    response = sys.stdin.readline()
    return response.rstrip('\\n')
builtins.input = custom_input
# --- End custom input handler ---

sys.path.insert(0, r"{self.project_path.as_posix()}")
...
'''
    ...
    def _run_subprocess(self, script: str) -> tuple[bool, str]:
        ...
        self.process = subprocess.Popen([python_path, '-u', '-c', script],
        ...
        for line in self.process.stdout:
            ...
            if stripped_line.startswith("__PYWORKS_INPUT_PROMPT__:"):
                prompt = stripped_line.split(":", 1)[1]
                self.input_required.emit(prompt)
                user_input = self.input_queue.get()
                self.process.stdin.write(user_input)
                self.process.stdin.flush()
            elif stripped_line.startswith("__PYWORKS_EXEC_NODE__"):
